...
import { listHeight } from "./list-geometry";
import { measureTextHeight } from "./text-style";
...
export function stackedHeight(el: MirElement, width: number, ctx: LowerCtx): number {
  switch (el.type) {
    case "text":
      return measureTextHeight(el, width, ctx);
    case "image": {
    ...
    case "ul":
    case "ol":
      return listHeight(el, width, ctx, stackedHeight);
    case "group":
...
function childIntrinsic(
  ...
  switch (el.type) {
    case "text": {
      if (isRow) {
        const nat = shapeText(el.text, el, Infinity, ctx.metrics);
        const w = Math.min(nat.width, mainExtent);
        const sh = shapeText(el.text, el, w, ctx.metrics);
        return { main: w, cross: sh.height };
      }
      // column: cross = box width, main = text's intrinsic height (rich-aware).
      return { main: measureTextHeight(el, crossExtent, ctx), cross: crossExtent };
    }
    case "image": {
      const img = ctx.images.get(el.src);
      const aspect = img && img.height > 0 ? img.width / img.height : 16 / 9;
